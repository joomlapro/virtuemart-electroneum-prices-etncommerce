...
class Vendor
{
   ...
   // const URL_QR = 'https://chart.googleapis.com/chart?cht=qr&chs=300x300&chld=L|0&chl=%s';
	...
	// qr.src = "" + encodeURI('https://link.electroneum.com/jWEpM5HcxP?vendor=' + code);
     ...
     * @return string
     */
    public function getQr($etn, $currency, $outlet, $paymentId = null)
    {
		
		 $this->etn = number_format(floatval($etn), 2, '.', '');
        // Convert the currency.
        //$etn = $this->currencyToEtn($amount, $currency);

        // Build the QR Code string.
        $qrCode = $this->getQrCode($this->etn, $outlet, $paymentId);
